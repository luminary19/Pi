...
describe("package commands", () => {
	...
	it.each([
		["default", ["update"]],
		["explicit self", ["update", "--self"]],
		["all", ["update", "--all"]],
		["forced self", ["update", "--self", "--force"]],
	])("refuses %s self-update before network or extension mutation", async (_label, args) => {
		const fakeNpmPath = join(tempDir, "fake-update-npm.cjs");
		const recordPath = join(tempDir, "unexpected-update.json");
		writeFileSync(
			fakeNpmPath,
		...
		try {
			...
			const stderr = errorSpy.mock.calls.map(([message]) => String(message)).join("\n");
			expect(stderr).toContain("self-update is disabled in the Lumicity fork");
			expect(stderr).toContain("scripts/update-pi-lumicity.ps1");
